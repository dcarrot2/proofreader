...
const mapOptionsToRuleset = (options: string[]) => {
  const map = {
    'conciseness': 'Be concise and to the point. Remove any unnecessary words or phrases but do not remove any information necessary to communicate the full intent of the text.',
    'grammar': 'Ensure the text is grammatically correct and follows the rules of the English language.',
    'style': 'Ensure the text is written in a style that is consistent with the intended audience.',
    'formatting': 'Ensure the text is formatted correctly and follows the rules of the English language.',
    'punctuation': 'Ensure the text is punctuated correctly and follows the rules of the English language.',
    'spelling': 'Ensure the text is spelled correctly and follows the rules of the English language.',
  }

  return options.map((option) => map[option as keyof typeof map]).filter(Boolean);
}
...
function TextArea() {
  return (
    <form action={submitText}>
      <textarea name="text" placeholder="Enter the piece of text to edit" />
